...
public class HistoriqueDataFetcher {
    ...
    public DataFetcher<Object> createHistorique(){
        return dataFetchingEnvironment -> {
            ...
            if(StringUtils.normalizeSpace(dataFetchingEnvironment.getArgument("date")).length() <= 0) return new Error("createHistorique", "INVALID_ARG", "Erreur : La date de l'historique que vous avez saisi : '" + dataFetchingEnvironment.getArgument("date") +  "' n'est pas correcte.");
            ...
            for(Question question : questions){
                if(question.getIntitule().equals(questionInput.get("intitule")) && questionInput.get("choixUnique").equals(question.isChoixUnique()) && question.getReponsesBonnes().size() == ((List<String>) questionInput.get("reponsesBonnes")).size() && question.getReponsesFausses().size() == ((List<String>) questionInput.get("reponsesFausses")).size() && questionInput.get("time").equals(question.getTime())){
                    ...
                    for(Historique historique : historiques) {
                        if (historique.getQuestion().getId_quest() == question.getId_quest() && dataFetchingEnvironment.getArgument("date").equals(historique.getDate())) return new Error("createHistorique", "ALREADY_EXISTS","Erreur : Un historique correspondant à cette question et cette date existe déjà.");
                    }
    ...
    public DataFetcher<Object> updateHistorique(){
        return dataFetchingEnvironment -> {
            ...
            Optional<Historique> historique = historiqueRepository.findById(Integer.parseInt(dataFetchingEnvironment.getArgument("id_hist")));
            if(!historique.isPresent()) return new Error("updateHistorique", "NOT_FOUND", "Erreur : Aucun historique correspondant à l'ID : '" + Integer.parseInt(dataFetchingEnvironment.getArgument("id_hist")) +  "' n'a été trouvé.");
            ...
            if(dataFetchingEnvironment.containsArgument("question")) {
                ...
                if(notFound) return new Error("updateHistorique", "NOT_FOUND", "Erreur : La question '" + questionInput.get("intitule") + "' n'existe pas.");
            }
            if(dataFetchingEnvironment.containsArgument("date")) {
                if(StringUtils.normalizeSpace(dataFetchingEnvironment.getArgument("date")).length() <= 0) return new Error("updateHistorique", "INVALID_ARG", "Erreur : La date de l'historique que vous avez saisi : '" + dataFetchingEnvironment.getArgument("date") +  "' n'est pas correcte.");
    ...
    public DataFetcher<Object> removeHistorique(){
        return dataFetchingEnvironment -> {
            // On vérifie que l'historique existe
            Optional<Historique> historique = historiqueRepository.findById(Integer.parseInt(dataFetchingEnvironment.getArgument("id_hist")));
            if(!historique.isPresent()) return new Error("removeHistorique", "NOT_FOUND", "Erreur : Aucun historique correspondant à l'ID : '" + Integer.parseInt(dataFetchingEnvironment.getArgument("id_hist")) +  "' n'a été trouvé.");
